...
class SSP_Admin {
	...
	public function __construct( $file, $version ) {
      ...
      add_filter( 'manage_series_custom_column' , array( $this, 'add_series_columns' ), 1, 3 );
			...
			// Series "archive" flag
			add_filter( 'series_edit_form_fields', array( $this, 'edit_series_archive_field') );
			add_filter( 'edited_series', array( $this, 'save_archive_on_edited_series' ) );
  ...
  /**
	...
	public function edit_series_archive_field($term) {
        ...
        <td>
					<input type="checkbox" name="is_archive" id="is_archive" value="yes" <?php if ($currently=="yes") { echo "checked='checked'"; } ?>/>
					<p class="description">Une émission archivée n'apparaît plus dans la liste normale des émissions sur la page "Podcasts" (il faut d'abord cliquer sur "Anciennes émissions")</p>
				</td>
    </tr><?php
	}
	...
	public function save_archive_on_edited_series($term_id) {
      ...
      update_term_meta( $term_id, 'is_archive', "yes" );
			...
			delete_term_meta( $term_id, 'is_archive');
		}
	}
    ...
    public function add_series_columns( $column_data , $column_name , $term_id ) {
        ...
        switch ( $column_name ) {
						case 'is_archive':
							$currently = get_term_meta( $term_id, 'is_archive', true );
							if ( $currently == "yes" ) {
								$column_data = "Ancienne émission";
							} else {
								$column_data = "Emission en cours";
							}
							break;
