...
class SSP_Frontend {
	...
	public function __construct( $file, $version ) {
		...
		$this->token = 'podcast';
		$this->series_list_attempted = false;
		$this->show_feed_attempted = false;
	...
	public function show_podcasts_list_header_block($query) {
		...
		if ( !is_home() && !is_single() && get_post_type() == 'podcast' && !is_search()) {
			if (empty($cat_title)) {
				$this->show_series_list();
			} else {
				$this->show_series_feeds();
			}
		}
	}


	private function show_series_feeds() {
		...
		$series_slug = $cat->slug;
		$series_prefix = $cat->taxonomy;

		$rss = '/?feed=podcast&podcast_series='.$series_slug;
		$itunes = 'itpc://'.$_SERVER['HTTP_HOST'].$rss;

		$rss_img = $this->assets_url.'img/rss.png';
		$itunes_img = $this->assets_url.'img/itunes.png';

		echo <<<END
		<div class="post-container">
			<div class="podcast type-podcast status-publish hentry">
...
END;
	...
	private function show_series_list() {
		$title = __( 'By series' , 'seriously-simple-podcasting' );
		$showArchive = __( 'Show archived series...' , 'seriously-simple-podcasting' );
...
END;
		$series = get_terms(array('taxonomy' => 'series'));
		foreach($series as $serie) {
			$url = '/'.$serie->taxonomy.'/'.$serie->slug;
			if ("yes" == get_term_meta( $serie->term_id, 'is_archive', true )) {
				$class = "class='seriesPodcast archivedSeries' style='display:none' ";
			} else {
				$class = "class='seriesPodcast'";
			}
			echo "<li $class><a href='$url'>".$serie->name."</a></li>";
		}

		$onclick="var a=document.getElementsByClassName('archivedSeries'); for (var i=0; i<a.length; i++) {a[i].style.display='block' ; }; jQuery('.posts').masonry(); return false;";

		echo <<<END
				</ul></div> <!-- /post-excerpt -->

				<div class="post-meta">
					<a href="#" onclick="$onclick">$showArchive</a>
					<div class="clear"></div>
				</div>
				<div class="clear"></div>
			</div> <!-- /post -->
...
END;
	...
	public function episode_meta_details ( $episode_id = 0, $context = 'content' ) {
		...
		if( $link && apply_filters( 'ssp_show_new_window_link', false ) ) {
